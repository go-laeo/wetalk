...
func (cc *CommentCreate) SetToID(id int) *CommentCreate {
	cc.mutation.SetToID(id)
	return cc
}

// SetNillableToID sets the "to" edge to the User entity by ID if the given value is not nil.
func (cc *CommentCreate) SetNillableToID(id *int) *CommentCreate {
	if id != nil {
		cc = cc.SetToID(*id)
	}
	return cc
...
func (cc *CommentCreate) SetTo(u *User) *CommentCreate {
	return cc.SetToID(u.ID)
}
...
func (cc *CommentCreate) createSpec() (*Comment, *sqlgraph.CreateSpec) {
	...
	if nodes := cc.mutation.ToIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.M2O,
			Inverse: false,
