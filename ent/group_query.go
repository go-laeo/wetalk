...
func (gq *GroupQuery) QueryPosts() *PostQuery {
	query := &PostQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GroupQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*Group, init func(*Group), assign func(*Group, *Post)) error {
	...
	query.Where(predicate.Post(func(s *sql.Selector) {
		s.Where(sql.InValues(group.PostsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "group_posts" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
