...
// HasFavoriteUsers applies the HasEdge predicate on the "favorite_users" edge.
func HasFavoriteUsers() predicate.Post {
	return predicate.Post(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(FavoriteUsersTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, FavoriteUsersTable, FavoriteUsersPrimaryKey...),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasFavoriteUsersWith applies the HasEdge predicate on the "favorite_users" edge with a given conditions (other predicates).
func HasFavoriteUsersWith(preds ...predicate.User) predicate.Post {
	return predicate.Post(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(FavoriteUsersInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2M, true, FavoriteUsersTable, FavoriteUsersPrimaryKey...),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
