...
// QueryFavoriteUsers chains the current query on the "favorite_users" edge.
func (pq *PostQuery) QueryFavoriteUsers() *UserQuery {
	query := &UserQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (pq *PostQuery) Clone() *PostQuery {
	...
	return &PostQuery{
		...
		withFavoriteUsers: pq.withFavoriteUsers.Clone(),
...
// WithFavoriteUsers tells the query-builder to eager-load the nodes that are connected to
// the "favorite_users" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *PostQuery) WithFavoriteUsers(opts ...func(*UserQuery)) *PostQuery {
	query := &UserQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withFavoriteUsers = query
	return pq
}
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	var (
		...
		loadedTypes = [4]bool{
			pq.withAuthor != nil,
			pq.withGroup != nil,
			pq.withFavoriteUsers != nil,
			pq.withComments != nil,
		}
	)
	if pq.withAuthor != nil || pq.withGroup != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withFavoriteUsers; query != nil {
		if err := pq.loadFavoriteUsers(ctx, query, nodes,
			func(n *Post) { n.Edges.FavoriteUsers = []*User{} },
			func(n *Post, e *User) { n.Edges.FavoriteUsers = append(n.Edges.FavoriteUsers, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *PostQuery) loadFavoriteUsers(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(post.FavoriteUsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(post.FavoriteUsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(post.FavoriteUsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(post.FavoriteUsersPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "favorite_users" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (pq *PostQuery) loadComments(ctx context.Context, query *CommentQuery, nodes []*Post, init func(*Post), assign func(*Post, *Comment)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "post_comments" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
