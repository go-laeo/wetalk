...
func (uq *UserQuery) loadFavoritePosts(ctx context.Context, query *PostQuery, nodes []*User, init func(*User), assign func(*User, *Post)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.FavoritePostsTable)
		s.Join(joinT).On(s.C(post.FieldID), joinT.C(user.FavoritePostsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.FavoritePostsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.FavoritePostsPrimaryKey[0]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "favorite_posts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
